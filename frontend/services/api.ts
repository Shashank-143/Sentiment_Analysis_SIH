...
export const processExcelFile = async (file: File): Promise<Blob> => {
  try {
    ...
    try {
      ...
      throw new Error(`Cannot connect to server (${errorMessage}). Please check your internet connection and ensure the backend server is running.`);
    }
    ...
    // Better handle network errors with type-safe error handling
    if (error instanceof Error && error.name === 'AbortError') {
